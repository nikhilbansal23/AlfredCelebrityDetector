...
def index():
    ...
    if request.method=='POST':
        if 'image' in request.files:
            ...
            if image_file:
                img_bytes, face_box = ImageHandler.process_image(image_file)
                player_info, player_name  = celebrity_detector.identify(img_bytes)

                if face_box is not None:
                    result_img_data = base64.b64encode(img_bytes).decode()
                else:
                    player_info = "No face detected in the image. Please try again with a different image."
    ...
    return render_template('index.html',player_info=player_info,result_img_data=result_img_data,answer=answer,user_question=user_question)
